...
ylFile = YlFile(os.path.abspath('.'))
redis_items = ylFile.getConfigDict("Redis-Config-pro")
...
class RedisUtil(object):
    def __init__(self, host=REDIS_HOST, port=REDIS_PORT, password=REDIS_PASSWORD, db=REDIS_DB):
        self.conn_pool = redis.ConnectionPool(host=host, port=port, max_connections=20, password=password, db=db)
        self.redis_db = redis.Redis(connection_pool=self.conn_pool)

...
class YlSpiderApi(scrapy.Spider):
                                    ...
                                    if not icsf:
                                        ...
                                        if phoenix_rule_id.__eq__("00000"):
                                            ...
                                            item = YlSpiderItem(
                                                ...
                                                insert_time=datetime.datetime.now(pytz.timezone('Asia/Shanghai')).strftime('%Y-%m-%d %H:%M:%S')
                                            )
                                        ...
                                        # 是产品
                                            ...
                                            form_data = {
                                                ...
                                                "ArrivalName": get_airport(_to_city_code, self.base_path).encode(
                                                    "utf-8").decode(
                                                    "latin1"),
                                                "Departure": __from_city_code,
                                                "DepartureDate": flight_time + " " + start_time,
                                                "DepartureName": get_airport(_from_city_code, self.base_path).encode(
                                                    "utf-8").decode(
                                                    "latin1"),
                                                "GetType": "0",
                                                "IsBaby": 0,
                                            ...
                                            yield scrapy.FormRequest(req_url, headers=headers,
                                                                     body=json.dumps(form_data),
                                                                     callback=self.parse_lowest_price, method="POST",
                                                                     dont_filter=True, meta=metadata)
    ...
    def parse_lowest_price(self, response):
        self.ylLog.info("***解析产品价格航班***")
        from_city = response.meta['from_city']
        from_city_code = response.meta['from_city_code']
        to_city = response.meta['to_city']
        to_city_code = response.meta['to_city_code']
        off_date = response.meta['off_date']
        create_time = response.meta['create_time']
        plane_no = response.meta['plane_no']
        plane_type = response.meta['plane_type']
        start_time = response.meta['start_time']
        end_time = response.meta['end_time']
        flight_transfer = response.meta['flight_transfer']
        flight_type = response.meta['flight_type']
        company = response.meta['company']
        company_no = response.meta['company_no']
        platform = response.meta['platform']
        from_city_airport = response.meta['from_city_airport']
        to_city_airport = response.meta['to_city_airport']
        if response.url == "**":
            msg = f"响应异常,OD为:{from_city}:{from_city_code}={to_city}:{to_city_code}\n"
            self.ylLog.exception(f"{msg}")
        elif response.status in [200, 201]:
            ...
            try:
                ...
                for i, economy_cabin in enumerate(economy_list):
                    ...
                    elif len(economy_cabin['limitLabel']) == 0 or (economy_cabin['limitLabel'][0]['name'] == "限时特惠" or
                ...
                if len(sell_price) == 0:
                    ...
                    print("from_city_code", from_city_code, "from_city", from_city, "to_city_code", to_city_code,
                          "to_city", to_city, "off_date", off_date,
                          "plane_no", plane_no, "price", price, "discount", discount, "from_city_airport",
                          from_city_airport, "to_city_airport", to_city_airport)
                    yield item
            except:
                print(traceback.print_exc())
                path = "/result/error_ua.txt"

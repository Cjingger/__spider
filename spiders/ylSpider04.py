...
import random
import requests.sessions as session
import pytz
import scrapy
import httpx
...
class YlSpider04(scrapy.Spider):
    name = 'ylSpider04'
    ...
    def parse_api(self, response):
        ...
        if self.redisUtil.hash_exist_key(f"ly:{self.now_date}-cookie", "cookies"):
            ...
            try:
                req_url = 'https://m.ly.com/kylintouch/flightApis/wx/flightquery/flights'
                with httpx.Client(headers=headers) as client:
                    ...
                    if res.status_code in [200, 201]:
                        ...
                        if self.has_node(data.get('body'), "fpc"):
                                    ...
                                    for flight in v:
                                        ...
                                        if not icsf:
                                            ...
                                            if phoenix_rule_id.__eq__("00000"):
                                                ...
                                                # 插入的city_code必须是处理后的city_code
                                                _data = [company, company_no, plane_no, start_time, end_time, from_city, _from_city_code, to_city, _to_city_code, create_time, str(discount), flight_time, platform, str(price), flight_type, plane_type, flight_transfer, from_city_airport, to_city_airport, self.task_time, ip_map[int(self.redis_items['db'])], count]
                            ...
                            names = ['company', 'company_no', 'plane_no', 'start_time', 'end_time', 'from_city', 'from_city_code', 'to_city', 'to_city_code', 'create_time', 'discount', 'off_date', 'platform', 'price', 'flight_type', 'plane_type', 'flight_transfer', 'from_city_airport', 'to_city_airport', 'task_time', 'server_ip', 'flight_number']
                            # 将csv文件放入logs目录下
                            csv_file = os.path.join(self.ylLog.second_path, fr"{_from_city_code}-{_to_city_code}-{flight_time}-{ip_map[int(self.redis_items['db'])]}.csv")
    ...
    def parse_lowest_price(self, meta_data: dict) -> list:
        ...
        if self.redisUtil.hash_exist_key(f"ly:{self.now_date}-cookie", "cookies"):
            ...
            try:
                    # res = session.session().post(url=req_url, datas=json.dumps(form_data), headers=headers, verify=False,
                    #                          allow_redirects=True, timeout=(30, 40))
                with httpx.Client(headers=headers) as client:
                    res = client.post(url=req_url, json=form_data, timeout=30, follow_redirects=False)
                    if res.status_code in [200, 201]:
                        ...
                        if len(sell_price) == 0:
                            ...
                            print("from_city_code", from_city_code, "from_city", from_city, "to_city_code",
                                  ...
                                  "plane_no", plane_no, "price", price, "discount", discount, "from_city_airport",
                                  from_city_airport, "to_city_airport", to_city_airport, "create_time", create_time)
                            return [company, company_no, plane_no, start_time, end_time, from_city, from_city_code, to_city, to_city_code, create_time, str(discount), off_date, platform, str(price), flight_type, plane_type, flight_transfer, from_city_airport, to_city_airport, self.task_time, ip_map[int(self.redis_items['db'])], count]
                    else:
                        self.fail += 1
                        msg = f"状态码异常>>>{self.fail}, 状态码: {res.status_code}\n"
                        self.ylLog.exception(msg)
